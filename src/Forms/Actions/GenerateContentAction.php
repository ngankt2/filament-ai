        ...
        return Action::make('generateContent')
            ...
            ->form([
                ...
                Select::make('template')
                    ...
                    ->options(function () {
                        return app(FilamentAI::class)->getContentTemplates();
                    })
                    ->reactive()
                    ->afterStateUpdated(function ($state, callable $set) {
                        $placeholders = app(FilamentAI::class)->getTemplatesPlaceholders();
                        if ($state) {
                            $placeholder = $placeholders[$state] ?? 'Write your content here';
                            $set('ai_prompt_placeholder', $placeholder);
                            $set('ai_prompt', app(FilamentAI::class)->getContentTemplates()[$state]??'');
                        }
                    })
                    ->visible(fn (callable $get) => !$get('use_existing_content')),
